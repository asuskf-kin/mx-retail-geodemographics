# %% [markdown]
# # 02 · Tiendas DENUE — canal Moderno vs Tradicional — ZM Mérida
#
...
# archivo de metadatos). Se identifica la **cadena** por nombre / razón social y se marcan las
...
print("Edición DENUE:", EDICION, "| establecimientos en Yucatán:", f"{len(denue):,}")

# %% [markdown]
# ## 2. Filtro SCIAN + ZM Mérida y limpieza
...
CADENAS = [
    ...
    ("7-Eleven", r"7[\s-]?ELEVEN|SEVEN ELEVEN|SIETE ONCE"),
    ("Circle K", r"CIRCLE\s*K"),
    ("Kiosko", r"\bKIOSKO\b"),
    ("Super Aki", r"\bAKI\b|SUPER\s*AKI|GRUPO AKI|DAC\b"),
    ("Six", r"\bSIX\b"),
    ("Bodega Aurrera", r"AURRERA"),
    ("Walmart", r"WAL\s*-?\s*MART|WALMART"),
    ("Sam's Club", r"\bSAM'?S\b"),
...
print(f"Tiendas ZM Mérida: {len(tiendas):,}")
    ...
    for url in ["https://overpass-api.de/api/interpreter", "https://overpass.kumi.systems/api/interpreter"]:
        try:
            r = requests.post(url, data={"data": q}, timeout=180,
                              headers={"User-Agent": "analisis-retail-merida/1.0"})  # Overpass exige User-Agent
            r.raise_for_status()
            el = r.json()["elements"]
            break
...
# %%
tiendas.to_parquet(C.PROC / "tiendas_denue_zm_merida.parquet", index=False)
for canal in C.CANALES:
    sub = tiendas[tiendas.canal == canal]
    out = C.OUT / f"02_tiendas_{canal.lower()}_zm_merida.xlsx"
